...
def register_callbacks(dash_app):
    ...
    filepath = os.path.join(base_dir, "dataset", "demo_3_refined.csv")
    # filepath = os.path.join(base_dir, "dataset", "demo_3.csv")
    # Read the CSV file
    ...
    @dash_app.callback(
        ...
        Output('keyword-store', 'data')],        # for Javascript access, storing search keywords in Store
    ...
    def fetch_data(n_clicks,start_date,end_date,radios,keyword):
        ...
        if n_clicks and start_date and end_date and keyword:
            try:
                ...
                dfs = dfc[['Date','Hour','Timestamp','Text','Speakers','media_link']]
                ...
                # Assuming your DataFrame is named 'filtered_df_rows' and the 'Text' column exists
                ...
                def highlight_text(text, search_terms):
                    ...
                    for term in search_terms:
                        ...
                        highlighted_text = re.sub(rf"\b({term})\b", f"<span>\\1</span>", highlighted_text, flags=re.IGNORECASE)
                ...
                filterable_columns = ['Date','Text']  
                
                # Dynamically create columnDefs with filtering for specified columns
                columnDefs = [
                    {"field": col, "filter": True} if col in filterable_columns else {"field": col} 
                ...
                grid = dag.AgGrid(
                        ...
                        columnSizeOptions={
                            ...
                            'columnLimits': [{'key': 'Text', 'minWidth': 650},
                                             {'key': 'Timestamp', 'minWidth': 150},
                                             {'key': 'Hour', 'minWidth': 70}],
                        },
                    )
                
                # tab.data takes out the props tag of stored table 
                return grid,new_placeholder,"",keyword  #Note the keyword from input has been send to Store via Output.
    ...
    def update_graph(selected_date, selected_segment,selected_topic):
        ...
        if selected_segment is None:  # Check for None explicitly
            ...
            dff = df[(df['Date']==selected_date) & (df.Hour==selected_segment)& (df.Topic==selected_topic)
                              ]
    ...
    def update_graph(selected_date, selected_segment,selected_topic):
        ...
        #
        # Combine path components (assuming dataset folder is one level down)
        filepath_speaker = os.path.join(base_dir, "dataset", "demo_3_refined.csv")
        # Read the CSV file
        df = pd.read_csv(filepath_speaker)
        
        # Selected Columns
        dfs = df[['id','Date','Hour','Timestamp','Text','Topic','Speakers','media_link']]
        ...
        if selected_segment is None:  # Check for None explicitly
            ...
            dff = dfs[(dfs['Date']==selected_date) & (dfs.Hour==selected_segment)& (dfs.Topic==selected_topic)]
            
        fig = go.Figure(data=[go.Table(
                header=dict(values=list(dff.columns),
                            fill_color='paleturquoise',
                            align='left'),
                cells=dict(values=[dff.id,dff.Date,dff.Hour, dff.Timestamp, dff.Text, dff.Topic, dff.Speakers, dff.media_link],
                           fill_color='lavender',
                           align='left'))
            ])
